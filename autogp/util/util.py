...
def get_flags():
    ...
    flags.DEFINE_integer('display_step', 1, 'Display progress every FLAGS.display_step iterations')
    flags.DEFINE_integer('mc_train', 200, 'Number of Monte Carlo samples used to compute stochastic gradients')
    flags.DEFINE_integer('mc_test', 500, 'Number of Monte Carlo samples for predictions and generalised nlpd')
    ...
    flags.DEFINE_float('opt_tol', 0.00001, 'Proportionate change objective threshold')
    flags.DEFINE_integer('num_components', 1, 'Number of mixture components on posterior')
    flags.DEFINE_boolean('diag_post', True, 'Diagonal approximate posterior distribution')
    flags.DEFINE_boolean('save_nlpds', False, 'Save nlpd for each predictive sample')
